...
2. Il les exploite librement dans le respect de la [licence](https://doc.transport.data.gouv.fr/presentation-et-mode-demploi-du-pan/conditions-dutilisation-des-donnees) (licence ouverte, licence ODbL etc.) ;
3. Il peut faire des retours à un producteur en commentaire du jeu de données (par exemple pour signaler une erreur dans un fichier) et [publier un jeu de données corrigé en ressource communautaire](https://doc.transport.data.gouv.fr/reutilisateurs/repartager-les-modifications-apportees-a-une-ressource), contribuant ainsi à améliorer la qualité globale des données.
...
* Une plateforme unique qui référence l’ensemble des jeux de données pertinents du territoire, avec des [outils pour faciliter la réutilisation](broken-reference) ;
* Des licences identifiées qui représentent une sécurité et un gain de temps (plus besoin de contractualiser individuellement avec chaque producteur) ;
...
1. Vous devez utiliser certains jeux de données référencés sur transport.data.gouv.fr et respecter les conditions de la licence indiquée.\
...
Si vous n'avez pas de compte, vous devez tout d'abord [vous créer un compte personnel ici, sur data.gouv.fr](https://www.data.gouv.fr/fr/register), puis vous pouvez l'associer à un profil Organisation relatif à votre structure.
